var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
var fromEvent = require('graphcool-lib').fromEvent;
module.exports = function (event) { return __awaiter(_this, void 0, void 0, function () {
    var graphcool, api, _a, guestGroupId, partnerName, partnerId, e_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 4, , 5]);
                graphcool = fromEvent(event);
                api = graphcool.api('simple/v1');
                _a = event.data, guestGroupId = _a.guestGroupId, partnerName = _a.partnerName;
                return [4 /*yield*/, createGuest(api, partnerName)];
            case 1:
                partnerId = (_b.sent()).createGuest.id;
                return [4 /*yield*/, getGuest(api, partnerId)];
            case 2:
                guest = _b.sent();
                return [4 /*yield*/, addPartnerToGuestGroup(api, guestGroupId, guest.Guest.id)];
            case 3:
                _b.sent();
                return [2 /*return*/, {
                        data: {
                            id: '0',
                        },
                    }];
            case 4:
                e_1 = _b.sent();
                console.log(e_1);
                return [2 /*return*/, { error: 'Wystąpił nieoczekiwany błąd' }];
            case 5: return [2 /*return*/];
        }
    });
}); };
function createGuest(api, name) {
    return __awaiter(this, void 0, void 0, function () {
        var mutation, variables;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    mutation = "\n    mutation CreateGuest($name: String!) {\n      createGuest(name: $name, isPresent: true, isVegetarian: false) {\n        id\n      }\n    }\n  ";
                    variables = {
                        name: name,
                    };
                    return [4 /*yield*/, api.request(mutation, variables)];
                case 1: return [2 /*return*/, _a.sent()];
            }
        });
    });
}
function getGuest(api, id) {
    return __awaiter(this, void 0, void 0, function () {
        var query, variables;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    query = "\n    query GetGuest($id: ID!) {\n      Guest(id: $id) {\n        id\n      }\n    }\n  ";
                    variables = {
                        id: id,
                    };
                    return [4 /*yield*/, api.request(query, variables)];
                case 1: return [2 /*return*/, _a.sent()];
            }
        });
    });
}
function addPartnerToGuestGroup(api, guestGroupId, partnerId) {
    return __awaiter(this, void 0, void 0, function () {
        var mutation, variables;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    mutation = "\n    mutation AddToGuestGroupOnGuest($guestGroupId: ID!, $partnerId: ID!) {\n        addToGuestGroupOnGuest(guestsGuestId: $partnerId, guestGroupGuestGroupId: $guestGroupId) {\n        guestsGuest {\n            id\n        }\n        guestGroupGuestGroup {\n            id\n          }\n      }\n    }\n  ";
                    variables = {
                        guestGroupId: guestGroupId,
                        partnerId: partnerId,
                    };
                    return [4 /*yield*/, api.request(mutation, variables)];
                case 1: return [2 /*return*/, _a.sent()];
            }
        });
    });
}
